...
for epoch in range(epochs):
    ...
    for data in train_data:
        optimizer.zero_grad()
        out1= model(data)
        loss1=loss_fn(out1[-train_test:,:,0],data.y[-train_test:,:,0])
        loss1.backward()
        optimizer.step()
        mape_avg.append(mape(out1[-train_test:,:,0],data.y[-train_test:,:,0]))
        rmse_avg.append(rmse(out1[-train_test:,:,0],data.y[-train_test:,:,0]))
    ...
    if epoch==84:
        break
print("训练完成！正在保存模型")
model_save_dir="data/user_data/graph_model/graph_active_model.pt"
